...
class MyDPOTrainer(DPOTrainer):
    ...
    def dpo_loss(
        ...
        if self.loss_type == 'tdpo':
            chosen_rewards = self.beta * (policy_chosen_logps.to(self.accelerator.device) - reference_chosen_logps.to(self.accelerator.device) + chosen_position_kl).detach()
            rejected_rewards = self.beta * (policy_rejected_logps.to(self.accelerator.device) - reference_rejected_logps.to(self.accelerator.device) + rejected_position_kl).detach()
        else:
            chosen_rewards = (
                self.beta
                * (
